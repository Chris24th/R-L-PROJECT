...
const Feed = ({ postDetails }) => {
    ...
    const onLike = async (e, id_post) => {
        let item = { id_user, id_post };
        await axios({
            method: "post",
            ...
            .then((response) => {
                // if (response.data.error) setLikeBtn(false);
                // else setLikeBtn(true);
                // api();
    ...
    const api = async () => {
        await axios({
            method: "get",
            url: "http://localhost/api/v1/displayreacts/",
        })
            .then((response) => {
            ...
            <div className="p-2 bg-white rounded m-5 mb-2 mt-2">
                <div className="d-flex flex-row justify-content-between align-items-start profile p-2">
                    ...
                    {post.username === user.username ? (
                        <Edit_DeleteModal_Post postData={post} />
                    ) : null}
                </div>
